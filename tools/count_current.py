...
def process_file_chunk(file_paths):
    """
    独立进程的 Worker 函数：一次性处理一大批文件，极大降低通信开销
    返回: (该批次的分类计数统计, 处理的文件数量)
    """
...
def scan_and_count():
    ...
    for phase in phases:
        ...
        # 收集所有文件的完整路径
        label_files = [os.path.join(labels_dir, f) for f in os.listdir(labels_dir) if f.endswith(".txt") and f != "classes.txt"]
