...
def worker_task(args):
    ...
    for _ in range(10):
        candidates.append((
            random.randint(0, max(1, bg_w - crop_w)), 
            random.randint(0, max(1, int(bg_h * 0.5) - crop_h))
        ))
    ...
    def run(self):
        ...
        for cls in classes_to_aug:
            ...
            if available == 0:
                ...
                print(f"⚠️ 类别 {cls} ({CURRENT_CLASSES[cls]}) 素材不足: 需要 {needed} 个，只有 {available} 个（会重复使用）")
        ...
        for cls in classes_to_aug:
            ...
            for _ in range(needed):
                ...
                task_args.append((
                    cls, mat_record_str, bg_lbl_path, bg_img_dir, 
                    out_img_path, out_lbl_path, self.max_iou
                ))
...
def parse_augment_config(augment_str):
    """
    解析增强配置字符串
    格式: "class_id:num,class_id:num,..."
    示例: "1:500,4:300,7:200"
    返回: [(class_id, num), ...]
    """
    targets = {}
...
使用示例:
  ...
  #
    ...
    parser.add_argument("--max_iou", type=float, default=0.0, 
                        help="最大允许的 IoU 阈值 (默认 0.0，即不允许任何重叠)")
    parser.add_argument("--workers", type=int, default=0, 
                        help="并行进程数 (0=自动推断为 cpu_count-2, 1=强制单线程)")
