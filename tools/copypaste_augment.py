...
2. 增量合并机制 (Additive Merge)：废弃覆盖逻辑。新图采用 {原图名}_aug_{任务ID}.jpg 命名。
3. 纯净度优先级遍历 (Sequential Purity Prioritization)：将背景严格按大类数量(0, 1...)排序。优先挨个消耗 0 大类的极品背景，用尽后再降级使用 1 大类背景。
4. 贪婪聚合：保持单图贴满5个目标，极大幅度压缩新增图片量。
"""
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="YOLO 智能按需 Copy-Paste 增强脚本 (V19)")
