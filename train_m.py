...
if __name__ == "__main__":
    ...
    model = YOLO("runs/detect/26m/train1/exp3/weights/last.pt")
    ...
    model.train(
        ...
        batch=16,
        pretrained=True,
        patience=30,  # EarlyStopping 的耐心值，连续 30 个 epoch 验证指标没有提升就停止训练
        cos_lr=True,
        save_period=5,
        ...
        scale=0.25,  # 启用 30% 的缩放增强（随机缩小或放大）
        # fliplr=0.304,
        ...
        perspective=0.0,
        # hsv_h=0.013,
        hsv_h=0.005,  # 绝对禁止！不能改变色相，绿灯不能变色
        hsv_s=0.453,
        hsv_v=0.294,
        bgr=0.0,
        # Loss Weights
        ...
        # cls=0.65,
        # dfl=0.96, # 分布焦点损失
        box=8.83,  # 从 9.83 适当下调。告诉网络：框画得差不多就行了，别死磕边缘了。
        cls=1.25,  # 从 0.65 提升两倍多！严厉警告网络：看清楚里面的箭头！认错形状后果很严重！
        dfl=0.96,  # 保持官方不变。红绿灯边缘清晰，配合端到端特性，这个值非常合理。
        # Optimizer and Learning Rate
        weight_decay=0.0003,  # 权重衰减惩罚项，用于防止过拟合。
        warmup_epochs=0.99,
